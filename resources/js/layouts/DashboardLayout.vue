            ...
            >
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative hidden md:block">
                        <label
                            class="flex h-9 items-center gap-2 rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 transition-colors focus-within:border-emerald-500/40 focus-within:bg-white/[0.04]"
                        >
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-3.5 w-3.5 shrink-0 text-white/40">
                            ...
                            <input
                                ...
                                class="w-44 bg-transparent text-xs text-white outline-none placeholder:text-white/30"
                            />
                        ...
                        <div v-if="searchOpen && q.trim().length >= 2"
                            class="absolute top-11 right-0 w-80 overflow-hidden rounded-xl border border-white/10 bg-[#111] shadow-2xl shadow-black/60">
                            <div v-if="!results.length" class="px-4 py-6 text-center text-xs text-white/30">Tidak ketemu "{{ q }}" di barang / pelanggan / supplier.</div>
                            <div v-else class="max-h-80 overflow-y-auto py-1.5">
                                <button v-for="(r, i) in results" :key="i" @click="go(r)"
                                    class="flex w-full items-center gap-2.5 px-3 py-2 text-left hover:bg-white/5">
                                    ...
                                    <span class="min-w-0 flex-1">
                                        ...
                                        <span class="block truncate text-[10px] text-white/40">{{ r.sub }}</span>
                                    </span>
                                    <span class="shrink-0 text-[9px] font-bold tracking-wider text-white/25 uppercase">{{ r.type }}</span>
                                </button>
                            </div>
                            <p class="border-t border-white/[0.06] px-3 py-1.5 text-[10px] text-white/25">Enter = buka hasil pertama • Esc = tutup</p>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        >
                            ...
                            <span v-if="alerts.length" class="absolute top-2 right-2.5 h-1.5 w-1.5 rounded-full bg-rose-500 ring-2 ring-[#0a0a0a]" />
                        </button>
                        <!-- dropdown ringkas -->
                        <div v-if="notifOpen"
                            class="absolute top-11 right-0 w-80 overflow-hidden rounded-xl border border-white/10 bg-[#111] shadow-2xl shadow-black/60">
                            <div class="flex items-center justify-between border-b border-white/[0.06] px-4 py-2.5">
                                <p class="text-xs font-black">Notifikasi <span class="text-white/30">({{ alerts.length }})</span></p>
                                <button @click="goNotif" class="text-[11px] font-bold text-emerald-400 hover:text-emerald-300">Lihat semua →</button>
                            </div>
                            <div class="max-h-80 overflow-y-auto py-1.5">
                                <button v-for="(a, i) in alerts.slice(0, 6)" :key="i" @click="go(a)"
                                    class="flex w-full items-start gap-2.5 px-4 py-2.5 text-left hover:bg-white/5">
                                    ...
                                    <span class="min-w-0 flex-1">
                                        ...
                                        <span class="block truncate text-[11px] text-white/40">{{ a.sub }}</span>
                                    </span>
                                </button>
                                <p v-if="!alerts.length" class="px-4 py-6 text-center text-xs text-white/30">Aman! Tidak ada peringatan. 🎉</p>
                            </div>
                        </div>
                    </div>
...
const results = computed(() => {
    ...
    for (const b of store.barangAktif.value) {
        if (b.nama.toLowerCase().includes(s) || b.barcode.includes(s))
            out.push({ type: 'produk', icon: '📦', cls: 'bg-amber-500/15 text-amber-300', label: b.nama, sub: `${b.barcode} • stok ${b.stok}`, href: '/produk' });
        if (out.length >= 5) break;
    }
    for (const p of store.pelangganAktif.value) {
        if (p.nama_pelanggan.toLowerCase().includes(s) || (p.telepon || '').includes(s))
            out.push({ type: 'pelanggan', icon: '👥', cls: 'bg-pink-500/15 text-pink-300', label: p.nama_pelanggan, sub: store.namaKelompokPelanggan(p.id_kelompok_pelanggan), href: '/pelanggan' });
        if (out.length >= 10) break;
    }
    for (const sp of store.supplierAktif.value) {
        if (sp.nama.toLowerCase().includes(s))
            out.push({ type: 'supplier', icon: '🚚', cls: 'bg-teal-500/15 text-teal-300', label: sp.nama, sub: sp.no_telepon || 'supplier', href: '/supplier' });
...
const alerts = computed(() => {
    ...
    if (role.value === 'kasir') {
        ...
        if (baru.length)
            out.push({ icon: '🆕', cls: 'bg-pink-500/15 text-pink-300', title: `${baru.length} pelanggan baru minggu ini`, sub: baru.slice(0, 2).map((p) => p.nama_pelanggan).join(', '), href: '/pelanggan' });
