        ...
        <template v-if="auth.role.value === 'kasir'">
            <div v-if="fil === 'all' || fil === 'pelanggan'" class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4 shadow-sm dark:shadow-none">
                ...
                <div class="mt-3 space-y-2">
                    <div v-for="p in pelangganBaru" :key="p.id_pelanggan" class="flex items-center gap-3 rounded-lg border border-slate-200 dark:border-white/[0.05] bg-white dark:bg-black/30 px-3 py-2.5 text-xs">
            ...
            <div v-if="fil === 'all' || fil === 'pelanggan'" class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4 shadow-sm dark:shadow-none">
                ...
                <div class="mt-3 space-y-2">
                    <div v-for="t in piutang" :key="t.id_penjualan" class="flex items-center gap-3 rounded-lg border border-slate-200 dark:border-white/[0.05] bg-white dark:bg-black/30 px-3 py-2.5 text-xs">
        ...
        <template v-else>
            <template v-if="fil === 'all' || fil === 'stok'">
                <div class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4 shadow-sm dark:shadow-none">
                    ...
                    <div class="mt-3 space-y-2">
                        <div v-for="b in stokList" :key="b.id_barang" class="flex items-center gap-3 rounded-lg border border-slate-200 dark:border-white/[0.05] bg-white dark:bg-black/30 px-3 py-2.5 text-xs">
            ...
            <template v-if="auth.role.value === 'super admin' && (fil === 'all' || fil === 'piutang')">
                <div class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4 shadow-sm dark:shadow-none">
                    <h2 class="text-sm font-bold">💸 Piutang belum bayar <span class="text-slate-400 dark:text-white/30">({{ piutang.length }}) • {{ formatRupiahShort(piutangTotal) }}</span></h2>
                    <div class="mt-3 space-y-2">
                        <div v-for="t in piutang" :key="t.id_penjualan" class="flex items-center gap-3 rounded-lg border border-slate-200 dark:border-white/[0.05] bg-white dark:bg-black/30 px-3 py-2.5 text-xs">
                            <div class="min-w-0 flex-1"><p class="truncate font-bold">#TRX-{{ String(t.id_penjualan).padStart(4, '0') }} • {{ store.namaPelanggan(t.id_pelanggan) }}</p><p class="text-[11px] text-slate-500 dark:text-white/40">{{ formatDateTime(t.tanggal_penjualan) }} • {{ t.cara_bayar }}</p></div>
                            <p class="shrink-0 font-black text-amber-700 dark:text-amber-400">{{ formatRupiah(t.total_faktur) }}</p>
            ...
            <template v-if="(fil === 'all' || fil === 'beli')">
                <div class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4 shadow-sm dark:shadow-none">
                    ...
                    <div class="mt-3 space-y-2">
                        <div v-for="p in draft" :key="p.id_pembelian" class="flex items-center gap-3 rounded-lg border border-slate-200 dark:border-white/[0.05] bg-white dark:bg-black/30 px-3 py-2.5 text-xs">
