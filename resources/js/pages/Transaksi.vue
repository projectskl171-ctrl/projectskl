    ...
    <div class="grid grid-cols-1 items-start gap-4 lg:grid-cols-12">
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2 2xl:grid-cols-3">
                <button v-for="b in filtered" :key="b.id_barang" @click="add(b)"
                    ...
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <span v-if="b.stok <= 10" class="shrink-0 rounded px-1 py-0.5 text-[9px] font-black" :class="b.stok === 0 ? 'bg-rose-500/20 text-rose-700 dark:text-rose-300' : 'bg-amber-500/20 text-amber-700 dark:text-amber-300'">{{ b.stok === 0 ? 'HABIS' : `Sisa ${b.stok}` }}</span>
                    </div>
                    <p class="mt-1 font-mono text-[10px] text-slate-400 dark:text-white/25">{{ b.barcode }}</p>
                    <div class="mt-2 flex items-center justify-between">
                        <p class="text-sm font-black text-emerald-700 dark:text-emerald-400">{{ formatRupiahShort(b.harga_jual) }}</p>
        ...
        <div class="h-fit rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-4 lg:sticky lg:top-20 lg:col-span-4">
            ...
            <div class="mt-2 flex gap-2 text-[11px] font-bold">
                <button @click="jenis = 'tunai'" :class="jenis === 'tunai' ? 'bg-emerald-500 text-white' : 'bg-slate-900/[0.04] dark:bg-white/5 text-slate-500 dark:text-white/50'" class="h-8 flex-1 rounded-lg">Tunai</button>
                <button @click="jenis = 'kredit'" :class="jenis === 'kredit' ? 'bg-amber-500 text-black' : 'bg-slate-900/[0.04] dark:bg-white/5 text-slate-500 dark:text-white/50'" class="h-8 flex-1 rounded-lg">Kredit</button>
            </div>
            ...
            <div class="mt-3 max-h-80 space-y-2 overflow-y-auto pr-0.5">
                <div v-for="l in cart" :key="l.id_barang" class="rounded-lg border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-black/30 p-2.5">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <button @click="remove(l.id_barang)" class="text-[11px] text-rose-600 dark:text-rose-400 hover:text-rose-500 dark:hover:text-rose-300">✕</button>
                    </div>
                    <div class="mt-2 flex items-center gap-1.5">
                        <button @click="dec(l)" class="h-7 w-7 rounded-md bg-slate-900/[0.06] dark:bg-white/10 text-sm font-black">−</button>
                        <span class="w-8 text-center text-sm font-black">{{ l.qty }}</span>
                        <button @click="inc(l)" class="h-7 w-7 rounded-md bg-slate-900/[0.06] dark:bg-white/10 text-sm font-black">+</button>
            ...
            <button @click="cart = []" class="mt-2 h-8 w-full rounded-lg bg-slate-900/[0.04] dark:bg-white/5 text-[11px] font-bold text-rose-700 dark:text-rose-300 hover:bg-rose-500/20">Batalkan Keranjang</button>
        </div>
    </div>
    ...
    <div v-if="struk" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" @click.self="struk = null">
        <div class="w-full max-w-xs rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111] p-5 font-mono text-xs">
            ...
            <div class="mt-4 flex gap-2 font-sans">
                <button @click="cetakUlang(struk)" class="h-9 flex-1 rounded-lg bg-slate-900/[0.06] dark:bg-white/10 text-xs font-black text-slate-900 dark:text-white hover:bg-white/15">🖨️ Cetak</button>
