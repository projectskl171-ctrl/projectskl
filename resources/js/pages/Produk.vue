            ...
            <table class="w-full min-w-[820px] text-left text-xs">
                ...
                <tbody>
                    <tr v-for="b in filtered" :key="b.id_barang" class="border-t border-slate-200 dark:border-white/[0.05] hover:bg-emerald-600/5 dark:hover:bg-white/[0.02]">
                        ...
                        <td class="px-3 py-2.5 text-right"><span class="rounded-md px-1.5 py-0.5 font-black" :class="b.stok === 0 ? 'bg-rose-500/15 text-rose-700 dark:text-rose-300' : b.stok <= 10 ? 'bg-amber-500/15 text-amber-700 dark:text-amber-300' : 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300'">{{ b.stok }}</span></td>
                        <td class="px-3 py-2.5 text-right whitespace-nowrap">
                            <button @click="edit(b)" class="rounded-md bg-slate-900/[0.04] dark:bg-white/5 px-2 py-1 text-[11px] font-bold hover:bg-slate-900/5 dark:hover:bg-white/10">Edit</button>
                            <button @click="store.deleteBarang(b.id_barang)" class="ml-1 rounded-md bg-slate-900/[0.04] dark:bg-white/5 px-2 py-1 text-[11px] font-bold text-rose-700 dark:text-rose-300 hover:bg-rose-500/20">Hapus</button>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <div v-if="show && auth.can('/produk')" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" @click.self="show = false">
        <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111] p-5">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2 text-xs">
                ...
                <label class="text-slate-500 dark:text-white/50">Harga beli<input v-model.number="form.harga_beli" type="number" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm text-slate-900 dark:text-white outline-none" /></label>
                <label class="text-slate-500 dark:text-white/50">Harga jual<input v-model.number="form.harga_jual" type="number" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm text-slate-900 dark:text-white outline-none" /></label>
                <label class="text-slate-500 dark:text-white/50">Stok<input v-model.number="form.stok" type="number" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm text-slate-900 dark:text-white outline-none" /></label>
