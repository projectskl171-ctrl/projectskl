<template>
    ...
    <div v-else class="space-y-4">
        <div v-if="auth.role.value === 'admin'" class="rounded-xl border border-violet-500/20 bg-violet-500/[0.06] px-4 py-3 text-xs text-slate-600 dark:text-white/70">
        ...
        <div v-else class="rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] px-4 py-3 text-xs text-slate-600 dark:text-white/70">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
            ...
            <select v-model="fRole" class="h-10 rounded-lg border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-black/40 px-3 text-sm outline-none">
        ...
        <div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-white/[0.06] shadow-sm dark:shadow-none">
            <table class="w-full min-w-[680px] text-left text-xs">
                ...
                <tbody>
                    <tr v-for="u in filtered" :key="u.id_user" class="border-t border-slate-200 dark:border-white/[0.05] hover:bg-emerald-600/5 dark:hover:bg-white/[0.02]">
                        ...
                        <td class="px-3 py-2.5 text-right"><button @click="toggle(u)" :disabled="!bisaUbah(u)" :class="[u.is_active ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-slate-900/[0.04] dark:bg-white/5 text-slate-500 dark:text-white/40', !bisaUbah(u) && 'cursor-not-allowed opacity-40']" class="rounded-md px-2 py-1 text-[11px] font-black">{{ u.is_active ? 'Aktif' : 'Nonaktif' }}</button></td>
                        <td class="px-3 py-2.5 text-right"><button v-if="bisaUbah(u)" @click="edit(u)" class="rounded-md bg-slate-900/[0.04] dark:bg-white/5 px-2 py-1 text-[11px] font-bold hover:bg-slate-900/5 dark:hover:bg-white/10">Edit</button><span v-else class="text-[10px] text-slate-400 dark:text-white/25">🔒</span></td>
                    </tr>
                </tbody>
            </table>
        </div>
        <p class="text-[11px] text-slate-400 dark:text-white/25">tb_user + roles • admin: CRUD kasir sekolahnya • super admin: CRUD admin & kasir + atur sekolahnya</p>
    </div>
    ...
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" @click.self="show = false">
        <div class="w-full max-w-md rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111] p-5 shadow-sm dark:shadow-none">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2 text-xs">
                ...
                <label v-if="auth.role.value === 'super admin'" class="block text-slate-500 dark:text-white/50">Role
                    <select v-model="form.id_role" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-2 text-sm text-slate-900 dark:text-white outline-none"><option :value="2">admin</option><option :value="3">kasir</option></select></label>
                <label v-if="auth.role.value === 'super admin'" class="block text-slate-500 dark:text-white/50">Sekolah
                ...
                <label v-else class="flex items-end pb-2 text-slate-500 dark:text-white/50"><span class="rounded bg-blue-500/15 px-2 py-1 text-[11px] font-black text-blue-700 dark:text-blue-300">Role: kasir (dikunci)</span></label>
                <label class="col-span-2 block text-slate-500 dark:text-white/50">Password {{ form.id_user ? '(kosongkan = tidak diubah)' : '' }}<input v-model="form.password" type="password" placeholder="••••••••" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm text-slate-900 dark:text-white outline-none" /></label>
            </div>
