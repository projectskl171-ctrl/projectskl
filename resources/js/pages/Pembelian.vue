            ...
            <div v-for="p in filtered" :key="p.id_pembelian" class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] shadow-sm dark:shadow-none">
                ...
                <div v-if="openId === p.id_pembelian" class="border-t border-slate-200 dark:border-white/[0.06] px-4 py-3 text-xs">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody>
                            <tr v-for="d in store.detailBeli(p.id_pembelian)" :key="d.id_detail_pembelian" class="border-t border-slate-200 dark:border-white/5">
                                ...
                                <td class="py-1.5 text-right text-slate-500 dark:text-white/50">{{ formatRupiah(d.harga_beli) }}</td>
                    ...
                    <div class="mt-3 flex gap-2">
                        ...
                        <button v-if="p.status_pembelian === 'draft'" @click="store.deletePembelian(p.id_pembelian)" class="h-9 rounded-lg bg-slate-900/[0.04] dark:bg-white/5 px-4 text-xs font-bold text-rose-700 dark:text-rose-300 hover:bg-rose-500/20">Hapus draft</button>
        ...
        <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111] p-5 shadow-sm dark:shadow-none">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
                ...
                <label class="col-span-2 text-xs text-slate-500 dark:text-white/50">Catatan
                    <input v-model="f.note" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm outline-none" placeholder="cth: Restock awal bulan" /></label>
            </div>
            <div class="mt-3 space-y-2">
                <div v-for="(l, i) in f.lines" :key="i" class="flex gap-2">
                    ...
                    <button @click="f.lines.splice(i, 1)" class="text-rose-600 dark:text-rose-400">✕</button>
                </div>
                <button @click="tambahBaris" class="h-9 w-full rounded-lg bg-slate-900/[0.04] dark:bg-white/5 text-xs font-bold text-slate-600 dark:text-white/60 hover:bg-slate-900/5 dark:hover:bg-white/10">+ Tambah baris</button>
            </div>
            ...
            <div class="mt-4 flex gap-2">
                <button @click="showForm = false" class="h-10 flex-1 rounded-lg bg-slate-900/[0.04] dark:bg-white/5 text-sm font-bold">Batal</button>
