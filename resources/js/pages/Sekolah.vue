<template>
    ...
    <div v-else class="space-y-4">
        <div class="grid grid-cols-1 gap-3 xl:grid-cols-2">
            <div v-for="s in sekolahList" :key="s.id_sekolah" class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-5">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button v-if="bisaToggle" @click="toggleAktif(s)" class="h-7 rounded-md bg-slate-900/[0.04] dark:bg-white/5 px-2.5 text-[11px] font-black text-slate-600 dark:text-white/70 hover:bg-slate-900/5 dark:hover:bg-white/10">{{ s.is_active ? 'Nonaktifkan' : 'Aktifkan' }}</button>
                    </div>
                </div>
                ...
                <label class="mt-2 block text-xs text-slate-500 dark:text-white/50">Nama sekolah<input v-model="s.nama_sekolah" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm font-bold text-slate-900 dark:text-white outline-none" /></label>
                <label class="mt-2 block text-xs text-slate-500 dark:text-white/50">Alamat<textarea v-model="s.alamat_sekolah" rows="2" class="mt-1 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 py-2 text-sm text-slate-900 dark:text-white outline-none"></textarea></label>
                <label class="mt-2 block text-xs text-slate-500 dark:text-white/50">Website<input v-model="s.website" class="mt-1 h-10 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black/50 px-3 text-sm text-slate-900 dark:text-white outline-none" /></label>
                <button @click="simpan(s)" class="mt-3 h-9 rounded-lg bg-slate-900/[0.04] dark:bg-white/5 px-4 text-xs font-black text-slate-600 dark:text-white/70 hover:bg-slate-900/5 dark:hover:bg-white/10">Simpan {{ s.nama_sekolah }}</button>
            </div>
        </div>
        ...
        <p class="text-[11px] text-slate-400 dark:text-white/25">tb_sekolah • kelola semua sekolah + toggle aktif/nonaktif (sekolah nonaktif hilang dari dashboard & scope).</p>
    </div>
</template>
